...
for N in Ns:
	...
	with open("eq_" + str(N) + "_u_over_reynolds_p1", "w") as file:
		for re in mean:
			file.write(str(re) + "\t" + str(mean[re][0]) + "\n")

	with open("eq_" + str(N) + "_v_over_reynolds_p1", "w") as file:
		for re in mean:
			file.write(str(re) + "\t" + str(mean[re][1]) + "\n")

	with open("eq_" + str(N) + "_u_over_reynolds_p2", "w") as file:
		for re in mean:
			file.write(str(re) + "\t" + str(mean[re][2]) + "\n")

	with open("eq_" + str(N) + "_v_over_reynolds_p2", "w") as file:
		for re in mean:
			file.write(str(re) + "\t" + str(mean[re][3]) + "\n")

	with open("eq_" + str(N) + "_u_over_reynolds_p3", "w") as file:
		for re in mean:
			file.write(str(re) + "\t" + str(mean[re][4]) + "\n")

	with open("eq_" + str(N) + "_v_over_reynolds_p3", "w") as file:
		for re in mean:
