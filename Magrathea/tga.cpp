...
int TGA::Load(char* szFilename) {
	...
	// Allocate some space
	// Check and clear pDat, just incase
	if(pData)
		delete [] pData;
...
int TGA::ReadHeader() { // Examine the header and populate our class attributes
...
int TGA::LoadRawData() { // Load uncompressed image data
	short iOffset;
...
int TGA::LoadTgaRLEData() { // Load RLE compressed image data
...
int TGA::LoadTgaPalette() { // Load a 256 color palette
	unsigned char bTemp;
...
void TGA::BGRtoRGB() { // Convert BGR to RGB (or back again)
...
void TGA::FlipImg() { // Flips the image vertically (Why store images upside down?)
	unsigned char bTemp;
...
unsigned char* TGA::GetImg() {
	return pImage;
}


unsigned char* TGA::GetPalette() {
	return pPalette;
}
